const DEFAULT_SWITCH_CLASSNAME =
  "_switch group px-1 relative inline-flex items-center justify-start shrink-0 overflow-hidden bg-mode-accent rounded-full w-12 h-7 transition-[background-color] data-[state=checked]:bg-[rgba(var(--button-bg))] data-[state=checked]:text-primary-foreground data-[state=checked]:bg-primary data-[invalid=true]:ring data-[invalid=true]:ring-destructive"
